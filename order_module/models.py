...
class Order(models.Model):
    ...
    def get_total_price(self):
        amount = 0
        # detail is closed(payed)
        if self.is_paid:
            for detail in self.orderdetail_set.all():
                amount += detail.final_price * detail.count
            if self.coupon_code is not None:
                amount -= (amount * self.coupon_code.amount) / 100
        # detail is open(not payed)
        else:
            for detail in self.orderdetail_set.all():
                amount += detail.product_detail.price * detail.count
            if self.coupon_code is not None:
                amount -= (amount * self.coupon_code.amount) / 100
        return amount
...
class OrderDetail(models.Model):
    ...
    final_price = models.IntegerField(null=True, help_text='the price will fix after payment', blank=True)
